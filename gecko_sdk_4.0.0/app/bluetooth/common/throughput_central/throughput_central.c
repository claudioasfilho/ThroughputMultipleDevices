  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_bt_evt_scanner_scan_report_id:
      if (process_scan_response(&(evt->data.evt_scanner_scan_report))) {
        ...
        //The code below searches for existing entries on the device list and only adds new discovered devices to it
        if (ThroughputReadyDevicesCounter > 0)
          {
            uint8_t deviceOnTheList = 0;
            for (uint8_t i = 0; i<ThroughputReadyDevicesCounter;i++)
              {
